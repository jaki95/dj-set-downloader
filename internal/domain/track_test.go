...
func TestTrackJSONSerialization(t *testing.T) {
	...
	expected := `{"artist":"Test Artist","title":"Test Title","start_time":"00:00:00","end_time":"00:05:00","track_number":1}`
	...
	assert.Equal(t, track.Title, newTrack.Title)
...
func TestTracklistJSONSerialization(t *testing.T) {
	...
	assert.Contains(t, jsonStr, `"title":"Track 1"`)
	assert.Contains(t, jsonStr, `"artist":"Artist 2"`)
	assert.Contains(t, jsonStr, `"title":"Track 2"`)
	...
	for i, track := range tracklist.Tracks {
		...
		assert.Equal(t, track.Title, newTracklist.Tracks[i].Title)
